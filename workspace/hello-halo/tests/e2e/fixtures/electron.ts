 ...
 * the ATUM CREA Electron application in E2E tests.
 *
 * Environment Variables:
 *   ATUM_TEST_API_KEY   - API key for testing (required for chat tests)
 *   ATUM_TEST_API_URL   - API URL (default: https://api.anthropic.com)
 *   ATUM_TEST_MODEL     - Model to use (default: claude-haiku-4-5-20251001)
 */
...
const TEST_API_KEY = process.env.ATUM_TEST_API_KEY || ''
const TEST_API_URL = process.env.ATUM_TEST_API_URL || 'https://api.anthropic.com'
const TEST_MODEL = process.env.ATUM_TEST_MODEL || 'claude-haiku-4-5-20251001'
...
function getAppPath(): string {
  ...
  if (process.platform === 'darwin') {
    ...
    const arm64Path = path.join(projectRoot, 'dist/mac-arm64/ATUM CREA.app/Contents/MacOS/ATUM CREA')
    const x64Path = path.join(projectRoot, 'dist/mac/ATUM CREA.app/Contents/MacOS/ATUM CREA')
    ...
    const winPath = path.join(projectRoot, 'dist/win-unpacked/ATUM CREA.exe')
    if (fs.existsSync(winPath)) {
      return winPath
    }
    throw new Error('Built app not found. Run "npm run build && npm run build:win" first.')
  } else {
    const linuxPath = path.join(projectRoot, 'dist/linux-unpacked/atum-crea')
...
function createTestConfigDir(appPath: string): string {
  const testDir = path.join(
    ...
    `atum-e2e-test-${Date.now()}`
  )

  // Create directory structure
  const atumDir = path.join(testDir, '.atum')
  const tempDir = path.join(atumDir, 'temp')
  const spacesDir = path.join(atumDir, 'spaces')

  fs.mkdirSync(testDir, { recursive: true })
  fs.mkdirSync(atumDir, { recursive: true })
  ...
  // Path: ~/Library/Application Support/ATUM CREA/headless-electron/electron-node
  if (process.platform === 'darwin') {
    const userDataDir = path.join(testDir, 'Library', 'Application Support', 'ATUM CREA')
