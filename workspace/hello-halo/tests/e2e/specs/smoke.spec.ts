...
async function navigateToChat(window: any) {
  ...
  // Look for "Enter ATUM CREA" or "进入 ATUM CREA" text button (supports both EN and CN)
  let enterAtum = await window.waitForSelector(
    'text=/Enter ATUM CREA|进入 ATUM CREA/i',
    { timeout: 5000 }
  ).catch(() => null)
...
test.describe('Smoke Tests', () => {
  ...
  test('shows splash or main content', async ({ window }) => {
    ...
    await Promise.race([
      ...
      window.waitForSelector('text=/ATUM CREA|API|连接|设置/', { timeout: 5000 }).catch(() => null)
    ])
...
test.describe('Core Features', () => {
  ...
  test('can send message and receive AI response', async ({ window }, testInfo) => {
    ...
    if (!hasApiKey()) {
      testInfo.skip(true, 'Skipping: ATUM_TEST_API_KEY not set')
      return
    }
    ...
    await window.waitForSelector('text=/ATUM CREA 工作中|ATUM CREA is working/i', { state: 'hidden', timeout: 45000 }).catch(() => {})
