 ...
 *   ATUM_TEST_API_KEY - API key for testing
 *   ATUM_TEST_API_URL - API URL (optional)
 *   ATUM_TEST_MODEL   - Model to use (optional)
 */
...
test.beforeEach(async ({}, testInfo) => {
  if (!hasApiKey()) {
    testInfo.skip(true, 'Skipping chat tests: ATUM_TEST_API_KEY not set')
  }
})

/**
 * Helper to navigate from Home Page to Chat Interface
 * The app shows Home Page first, need to click "进入 ATUM CREA" to get to chat
 */
async function navigateToChat(window: any) {
  await window.waitForSelector('#root', { timeout: 10000 })
  await window.waitForLoadState('networkidle')

  // Look for "进入 ATUM CREA" text button (the main ATUM CREA temp space card)
  // Try multiple approaches
  let enterAtum = await window.waitForSelector(
    'text=/进入 ATUM CREA/',
    { timeout: 5000 }
  ).catch(() => null)

  if (!enterAtum) {
    // Fallback: look for any element with "ATUM CREA" text that's clickable
    enterAtum = await window.waitForSelector(
      ':text("ATUM CREA"):visible',
      { timeout: 5000 }
    ).catch(() => null)
  }

  if (enterAtum) {
    await enterAtum.click()
  }
...
test.describe('Chat Interface', () => {
  test('chat input is visible and functional', async ({ window }) => {
    ...
    await chatInput.fill('Hello, ATUM CREA!')
    const value = await chatInput.inputValue()
    expect(value).toBe('Hello, ATUM CREA!')
  })
...
test.describe('Real Chat Flow', () => {
  ...
  test('can send message and receive response', async ({ window }) => {
    ...
    // Wait for AI to finish working (wait for "ATUM CREA 工作中" to disappear)
    await window.waitForSelector(
      'text="ATUM CREA 工作中"',
  ...
  test('displays thinking indicator during response', async ({ window }) => {
    ...
    // Look for working indicator ("ATUM CREA 工作中")
    ...
    await window.waitForSelector('text="ATUM CREA 工作中"', { state: 'hidden', timeout: 45000 }).catch(() => {})
