...
declare global {
  var __ATUM_TEST_DIR__: string
}

globalThis.__ATUM_TEST_DIR__ = ''

// Create a unique temporary directory for each test
function createTestDir(): string {
  const dir = path.join(
    os.tmpdir(),
    'atum-test-' + Date.now() + '-' + Math.random().toString(36).slice(2)
  )
  globalThis.__ATUM_TEST_DIR__ = dir
  return dir
}
...
vi.mock('os', async (importOriginal) => {
  ...
  return {
    ...actual,
    homedir: () => globalThis.__ATUM_TEST_DIR__ || '/tmp/atum-test-fallback'
  }
})
...
vi.mock('electron', () => {
  return {
    app: {
      getPath: (name: string) => {
        const dir = globalThis.__ATUM_TEST_DIR__ || '/tmp/atum-test-fallback'
        if (name === 'home') return dir
        if (name === 'userData') return path.join(dir, '.atum')
        return dir
      },
      setLoginItemSettings: vi.fn(),
      getLoginItemSettings: vi.fn(() => ({ openAtLogin: false })),
      getName: vi.fn(() => 'ATUM CREA'),
...
afterEach(() => {
  const testDir = globalThis.__ATUM_TEST_DIR__
...
export function getTestDir(): string {
  return globalThis.__ATUM_TEST_DIR__
}
