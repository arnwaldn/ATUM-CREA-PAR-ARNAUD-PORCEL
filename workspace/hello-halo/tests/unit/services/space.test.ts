...
describe('Space Service', () => {
  ...
  describe('getAtumCreSpace', () => {
    it('should return the ATUM CREA temp space', () => {
      const atumSpace = getAtumCreSpace()

      expect(atumSpace.id).toBe('atum-temp')
      expect(atumSpace.name).toBe('ATUM CREA')
      expect(atumSpace.isTemp).toBe(true)
      expect(atumSpace.icon).toBe('sparkles')
    })

    it('should have valid path', () => {
      const atumSpace = getAtumCreSpace()

      expect(atumSpace.path).toBeTruthy()
      expect(fs.existsSync(atumSpace.path)).toBe(true)
    })
  ...
  describe('createSpace', () => {
    ...
    it('should create app data directory inside space', async () => {
      const space = await createSpace({
        name: 'Test Space',
        icon: 'folder'
      })

      const appDir = path.join(space.path, '.atum')
    ...
    it('should create meta.json with space info', async () => {
      ...
      const metaPath = path.join(space.path, '.atum', 'meta.json')
    ...
    it('should handle custom path', async () => {
      ...
      expect(fs.existsSync(path.join(customPath, '.atum', 'meta.json'))).toBe(true)
    })
  })
  ...
  describe('getSpace', () => {
    ...
    it('should return ATUM CREA space for halo-temp id', () => {
      const space = getSpace('atum-temp')

      expect(space).toBeDefined()
      expect(space?.id).toBe('atum-temp')
      expect(space?.isTemp).toBe(true)
    })
  })

  describe('deleteSpace', () => {
    it('should delete space and its app data directory', async () => {
      const space = await createSpace({
        name: 'Delete Test',
        icon: 'folder'
      })
      ...
      // App data directory should be deleted, but space directory may remain (for custom paths)
      expect(fs.existsSync(appDir)).toBe(false)
    })

    it('should not allow deleting ATUM CREA temp space', async () => {
      // deleteSpace may return false or throw for temp space
      try {
        const result = await deleteSpace('atum-temp')
