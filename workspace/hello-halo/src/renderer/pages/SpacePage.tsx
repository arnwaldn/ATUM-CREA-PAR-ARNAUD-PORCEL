...
export function SpacePage() {
  ...
  const { currentSpace, atumSpace, loadAtumCreSpace, setCurrentSpace: setSpaceStoreCurrentSpace, refreshCurrentSpace, openSpaceFolder } = useSpaceStore()
  const {
    currentSpaceId,
    setCurrentSpace,
    getConversations,
  ...
  // Auto-load atumSpace when no space is selected (direct-to-chat mode)
  useEffect(() => {
    if (!currentSpace) {
      if (atumSpace) {
        // atumSpace already loaded, set it as current
        setSpaceStoreCurrentSpace(atumSpace)
        setCurrentSpace(atumSpace.id)
      } else {
        // Load atumSpace from backend
        loadAtumCreSpace().then(() => {
          const loaded = useSpaceStore.getState().atumSpace
          if (loaded) {
            useSpaceStore.getState().setCurrentSpace(loaded)
            useChatStore.getState().setCurrentSpace(loaded.id)
          }
        })
      }
    }
  }, [currentSpace, atumSpace])
  ...
  // Early return AFTER all hooks (React requires hooks to be called in consistent order)
  if (!currentSpace) {
    return (
    ...
    <div className="h-full w-full flex flex-col">
      ...
      */}
      ...
      <Header
        left={
          <>
            {/* Back button - only show for dedicated spaces (temp space has no home to go back to) */}
            {!currentSpace.isTemp && (
              <button
                onClick={handleBack}
            ...
            <SpaceIcon iconId={currentSpace.icon} size={70} className="flex-shrink-0" />
            <span className="font-medium text-sm truncate max-w-[100px] sm:max-w-[200px] hidden sm:inline">
              {currentSpace.isTemp ? 'ATUM CREA' : currentSpace.name}
            </span>
            ...
            {conversations.length > 0 && (
              <div className="ml-1">
                <ChatHistoryPanel
                  ...
                  spaceName={currentSpace.isTemp ? 'ATUM CREA' : currentSpace.name}
