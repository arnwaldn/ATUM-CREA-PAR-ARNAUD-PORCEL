/**
 * HTML Viewer - HTML preview with source toggle and inspector
 *
 * Features:
 ...
 * - Inspector panel: hover/click elements, view styles, console output
 */

import { useState, useRef, useMemo, useEffect, useCallback } from 'react'
import { Copy, Check, Code, Eye, ExternalLink, Globe, Search } from 'lucide-react'
import { highlightCodeSync } from '../../../lib/highlight-loader'
import { useTranslation } from '../../../i18n'
import { api } from '../../../api'
import { useCanvasStore, type CanvasTab } from '../../../stores/canvas.store'
import { getInspectorScript } from './inspector-inject'
import { HtmlInspector, type InspectorElement, type ConsoleEntry } from './HtmlInspector'
...
export function HtmlViewer({ tab }: HtmlViewerProps) {
  ...
  // Build srcdoc with optional inspector script
  const srcdocContent = useMemo(() => {
    if (!inspectorEnabled) return content
    // Inject inspector script before closing </head> or at start of content
    const headCloseIdx = content.indexOf('</head>')
    if (headCloseIdx >= 0) {
      return content.slice(0, headCloseIdx) + getInspectorScript() + content.slice(headCloseIdx)
    }
    // No <head> tag — inject at the very start
    return getInspectorScript() + content
  }, [content, inspectorEnabled])
  ...
  const handleMessage = useCallback((event: MessageEvent) => {
    ...
    switch (type) {
      case 'inspector:hover':
        setHoveredElement(data as InspectorElement | null)
        break
      case 'inspector:select':
        setSelectedElement(data as InspectorElement)
        break
      case 'inspector:console':
        setConsoleEntries(prev => [...prev.slice(-199), data as ConsoleEntry])
        break
    }
  }, [])
  ...
  }, [inspectorEnabled, handleMessage])

  // Reset inspector state when toggling off
    ...
    <div className="relative flex flex-col h-full bg-background">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border bg-card/50">
        ...
        <div className="flex items-center gap-1">
          {/* Inspector toggle (only in preview mode) */}
          {viewMode === 'preview' && (
            <button
              ...
              title={inspectorEnabled ? t('Close Inspector') : t('Open Inspector')}
            >
          ...
          {/* Separator */}
          {viewMode === 'preview' && <div className="w-px h-4 bg-border/50 mx-0.5" />}
      ...
      <div className={`flex-1 min-h-0 overflow-hidden flex flex-col`}>
        {/* Preview / Source area */}
        <div className={`${inspectorEnabled && viewMode === 'preview' ? 'flex-[2]' : 'flex-1'} min-h-0 overflow-hidden`}>
          {viewMode === 'preview' ? (
            <iframe
              ref={iframeRef}
        ...
        {/* Inspector panel (only when enabled and in preview mode) */}
        {inspectorEnabled && viewMode === 'preview' && (
          <div className="flex-1 min-h-[120px] max-h-[40%]">
            <HtmlInspector
