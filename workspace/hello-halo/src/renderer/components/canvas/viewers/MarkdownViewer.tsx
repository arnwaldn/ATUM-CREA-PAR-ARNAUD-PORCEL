...
/**
 * Resolve relative image paths to atum-file:// protocol URLs
...
function resolveImageSrc(src: string | undefined, basePath: string): string {
  ...
  // Resolve relative paths to atum-file:// protocol
  if (src.startsWith('./')) {
    return `atum-file://${basePath}/${src.slice(2)}`
  }
  ...
  if (src.startsWith('../')) {
    ...
    return `atum-file://${parts.join('/')}/${srcParts.join('/')}`
  }

  if (src.startsWith('/')) {
    return `atum-file://${src}`
  }

  // Relative path without prefix
  return `atum-file://${basePath}/${src}`
}
          ...
          <div className="prose prose-invert max-w-none p-6 sm:p-8">
            <ReactMarkdown
              ...
              components={{
                ...
                // Style images - resolve relative paths using atum-file:// protocol
                img({ src, alt }) {
                  return (
                    <img
