...
import { api } from '../../api'
import type { AtumCreConfig } from '../../types'
...
export function SystemSection({ config, setConfig }: SystemSectionProps) {
  ...
  const [agentTeams, setAgentTeams] = useState(config?.system?.agentTeams !== false)
  ...
  // Handle agent teams change
  const handleAgentTeamsChange = async (enabled: boolean) => {
    setAgentTeams(enabled)
    if (config) {
      const partialConfig = { system: { ...config.system, agentTeams: enabled } }
      const newConfig = { ...config, ...partialConfig } as AtumCreConfig
      setConfig(newConfig)
      try {
        await api.setConfig(partialConfig)
      } catch (error) {
        console.error('[SystemSection] Failed to save agent teams setting:', error)
        setAgentTeams(!enabled) // Revert on error
      }
    }
  }
    ...
    <>
      ...
      <section id="permissions" className="bg-card rounded-xl border border-border p-6">
        ...
        <div className="bg-muted/50 rounded-lg p-3 mb-4 text-sm text-muted-foreground">
          {t('We recommend full trust mode - use natural language to control ATUM CREA.')}
        </div>
        ...
        <div className="space-y-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                ...
                <span
                  ...
                  title={t('Automatically run ATUM CREA when system starts')}
                >
                  ?
                </span>
              </div>
              <p className="text-sm text-muted-foreground">
                {t('Automatically run ATUM CREA when system starts')}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-border">
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground">
                {t('Allow AI to coordinate multiple agents for complex tasks')}
              </p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                checked={agentTeams}
                onChange={(e) => handleAgentTeamsChange(e.target.checked)}
                className="sr-only peer"
              />
              <div className="w-11 h-6 bg-secondary rounded-full peer peer-checked:bg-primary transition-colors">
                <div
                  className={`w-5 h-5 bg-white rounded-full shadow-md transform transition-transform ${
                    agentTeams ? 'translate-x-5' : 'translate-x-0.5'
                  } mt-0.5`}
                />
              </div>
            </label>
