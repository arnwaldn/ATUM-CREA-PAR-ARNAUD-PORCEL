...
/**
 * AtumScarabIcon - Drop-in replacement for Sparkles icon
 * Renders the ATUM golden scarab beetle as an inline icon.
 * Accepts className and size props like Lucide icons.
 */
function AtumScarabIcon({ className = '', size = 20 }: { className?: string; size?: number }) {
  return (
    <img
      src={atumLogoSrc}
      alt="ATUM"
...
// Re-export the scarab as "Sparkles" so existing consumers get the scarab
export const Sparkles = AtumScarabIcon
...
export const spaceIconMap: Record<string, LucideIcon> = {
  ...
  rocket: Rocket,
  star: Star,
  sparkles: LucideSparkles, // placeholder, SpaceIcon handles this case specially
}
...
export const spaceIconColors: Record<string, string> = {
  ...
  sparkles: 'text-primary',        // ATUM CREA brand color
}
...
export function SpaceIcon({ iconId, className = '', size = 20, colored = true }: SpaceIconProps) {
  // Special case: render ATUM scarab for sparkles icon
  if (iconId === 'sparkles') {
    return <AtumScarabIcon className={className} size={size} />
  }
...
// Re-export additional icons (Sparkles is already exported as AtumScarabIcon above)
export {
  Hand,
  Settings,
  Plus,
