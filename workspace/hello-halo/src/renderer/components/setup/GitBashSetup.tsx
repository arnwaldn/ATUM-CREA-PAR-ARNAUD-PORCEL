...
export function GitBashSetup({ onComplete }: GitBashSetupProps) {
  ...
  const handleContinue = async () => {
    ...
    try {
      const result = await window.atumCrea.installGitBash((progressData: DownloadProgress) => {
  ...
  const handleManualInstall = () => {
    window.atumCrea.openExternal?.('https://git-scm.com/downloads/win')
  }
      ...
      <div className="fixed inset-0 bg-background flex items-center justify-center overflow-auto">
        <div className="w-[520px] p-8 rounded-2xl bg-card border border-border shadow-xl">
          <div className="flex items-center gap-3 mb-6">
            <AtumCreLogo size={40} animated={false} />
