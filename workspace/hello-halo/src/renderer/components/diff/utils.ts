...
/**
 * Extract file changes from ALL messages in a conversation.
 * Aggregates Write/Edit tool calls across every assistant message.
 */
export function extractConversationFileChanges(messages: Message[]): FileChanges {
  const allThoughts: Thought[] = []
  for (const msg of messages) {
    if (msg.role === 'assistant' && msg.thoughts) {
      allThoughts.push(...msg.thoughts)
    }
  }
  return extractFileChanges(allThoughts)
}
