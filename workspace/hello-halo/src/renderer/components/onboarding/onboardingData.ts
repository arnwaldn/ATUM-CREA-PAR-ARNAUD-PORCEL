 ...
 * - AI response text
 * - HTML artifact content (a beautiful PPT introducing ATUM CREA)
 *
...
export const ONBOARDING_ARTIFACT_NAME = 'atum-intro.html'
...
export function getOnboardingPrompt(translator?: TFunction): string {
  ...
  return t(
    'Help me create an HTML slide deck introducing ATUM CREA (1920x1080) with a clean, modern style. Include: 1) What is ATUM CREA 2) Core capabilities 3) Use cases 4) Getting started'
  )
}
...
export function getOnboardingAiResponse(translator?: TFunction): string {
  const t = getTranslator(translator)
  return t(
    "Sure, I'll create a polished HTML presentation that introduces ATUM CREA's core features and how to use it. The slides use a modern, minimal design, support left/right arrow navigation, and fit 1920x1080."
  )
}

export function getOnboardingConversationTitle(translator?: TFunction): string {
  const t = getTranslator(translator)
  return t('Welcome to ATUM CREA')
}

export function getOnboardingHtmlArtifact(translator?: TFunction, lang: string = getCurrentLanguage()): string {
  const t = getTranslator(translator)

  const docTitle = t('ATUM CREA - AI that gets things done')
  const tagline = t('AI that gets things done')
  const whatIsAtumCrea = t('What is ATUM CREA?')
  const moreThanChatTitle = t('More than chat')
  const moreThanChatDesc = t('ATUM CREA understands your goals, plans tasks, and can execute autonomously—not just answer questions.')
  ...
  const naturalLangDesc = t('Describe what you want in plain language; ATUM CREA will understand and act.')
  ...
  const writeCodeDesc = t('Describe your needs and ATUM CREA will generate complete code across languages and frameworks.')
  ...
  const webDesc = t('Need fresh information? ATUM CREA can search online and summarize what matters.')
  const docsTitle = t('Write documents')
  const docsDesc = t('Reports, proposals, emails—tell ATUM CREA what you need and it will draft it.')
  ...
  const startText = t('Start using ATUM CREA')
...
<body>
  <div class="slides">
    ...
    <!-- Slide 2: What is ATUM CREA -->
    <section class="slide slide-content">
      <h2>${whatIsAtumCrea}</h2>
      <div class="features">
        <div class="feature">
