...
// Small inline component to render the ATUM scarab as an icon-like element
...
const STEP_CONFIGS = {
  'atum-space': {
    targetSelector: '[data-onboarding="atum-space"]',
    tooltipPosition: 'bottom' as const,
    titleKey: 'Start your first conversation',
    descriptionKey: 'Click to enter ATUM CREA space and experience how AI can help you',
    icon: AtumIcon,
    iconColor: '',
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center gap-2 mt-4 pt-3 border-t border-border">
        {['atum-space', 'send-message', 'view-artifact'].map((step, index) => (
          <div
            key={step}
            className={`w-2 h-2 rounded-full transition-colors ${
              step === currentStep
                ? 'bg-primary'
                : index < ['atum-space', 'send-message', 'view-artifact'].indexOf(currentStep)
                ? 'bg-primary/50'
                : 'bg-muted'
            }`}
          />
        ...
        <span className="text-xs text-muted-foreground ml-2">
          {['atum-space', 'send-message', 'view-artifact'].indexOf(currentStep) + 1} / 3
        </span>
      </div>
    </Spotlight>
  )
