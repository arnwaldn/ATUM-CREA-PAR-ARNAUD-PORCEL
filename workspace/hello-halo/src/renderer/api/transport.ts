...
// Detect if running in Electron (has window.atumCrea via preload)
export function isElectron(): boolean {
  return typeof window !== 'undefined' && 'atumCrea' in window
}
...
export function getAuthToken(): string | null {
  if (typeof localStorage !== 'undefined') {
    return localStorage.getItem('atum_remote_token')
  }
  return null
}
...
export function clearAuthToken(): void {
  if (typeof localStorage !== 'undefined') {
    localStorage.removeItem('atum_remote_token')
  }
}
...
export async function httpRequest<T>(
  ...
  try {
    ...
    if (response.status === 401) {
      ...
      document.cookie = 'atum_authenticated=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;'
...
export function onEvent(channel: string, callback: (data: unknown) => void): () => void {
  if (isElectron()) {
    // Use IPC in Electron
    const methodMap: Record<string, keyof typeof window.atumCrea> = {
    ...
    if (method && typeof window.atumCrea[method] === 'function') {
      return (window.atumCrea[method] as (cb: (data: unknown) => void) => () => void)(callback)
    }
