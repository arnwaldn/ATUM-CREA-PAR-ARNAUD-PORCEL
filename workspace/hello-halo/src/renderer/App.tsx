...
import { api } from './api'
import type { AgentEventBase, Thought, ToolCall, AtumCreConfig, AgentErrorType } from './types'
...
/**
 * Play a prominent multi-note notification melody using Web Audio API.
 * Designed to be audible from another room — louder, longer (~3s), with
 * a distinctive ascending pattern played twice for attention.
 * No external files needed — synthesized in real-time.
 */
function playNotificationChime() {
  try {
    ...
    // Master gain — loud enough to hear from another room
    const master = ctx.createGain()
    master.gain.setValueAtTime(0.85, now)
    master.connect(ctx.destination)

    // Helper: play a single note with attack/sustain/release envelope
    function playNote(freq: number, startTime: number, duration: number, volume: number) {
      ...
      // Add slight harmonic richness with a second oscillator
      ...
      gain.gain.linearRampToValueAtTime(volume, startTime + 0.015) // fast attack
      gain.gain.setValueAtTime(volume * 0.8, startTime + duration * 0.3) // sustain
    ...
    // Melody: C5 → E5 → G5 → C6 (ascending major arpeggio), played twice
    const notes = [
      { freq: 523.25, vol: 0.7 },  // C5
      { freq: 659.25, vol: 0.75 }, // E5
      { freq: 783.99, vol: 0.8 },  // G5
      { freq: 1046.5, vol: 0.85 }  // C6
    ]
    ...
    for (let i = 0; i < notes.length; i++) {
      ...
      playNote(notes[i].freq, start, noteDuration + 0.15, notes[i].vol)
    }

    // Brief pause then second pass (slightly higher pitch for urgency)
    const secondPassStart = now + notes.length * (noteDuration + noteGap) + 0.4
    for (let i = 0; i < notes.length; i++) {
      const start = secondPassStart + i * (noteDuration + noteGap)
      playNote(notes[i].freq * 1.06, start, noteDuration + 0.15, notes[i].vol)
    }

    // Total duration ~3.4s — cleanup after sound finishes
...
function applyTheme(theme: 'light' | 'dark' | 'system') {
  ...
  try {
    localStorage.setItem('atum-theme', theme)
  } catch (e) { /* ignore */ }
...
export default function App() {
  ...
  const { spaces, atumSpace, setCurrentSpace: setSpaceStoreCurrentSpace } = useSpaceStore()
  ...
  useEffect(() => {
    ...
    const unsubComplete = api.onAgentComplete((data) => {
      ...
      if (currentConfig?.appearance?.notificationSound !== false) {
    ...
    const handleNavigateToResult = async (event: Event) => {
      ...
      try {
        ...
        if (spaceId !== currentSpaceId) {
          ...
          if (spaceId === 'atum-temp' && atumSpace) {
            targetSpace = atumSpace
          } else {
  ...
  }, [currentSpaceId, spaces, atumSpace, setSpaceStoreCurrentSpace, setChatCurrentSpace, loadConversations, selectConversation])
  ...
  const handleGitBashSetupComplete = async (installed: boolean) => {
    ...
    if (response.success && response.data) {
      const loadedConfig = response.data as AtumCreConfig
