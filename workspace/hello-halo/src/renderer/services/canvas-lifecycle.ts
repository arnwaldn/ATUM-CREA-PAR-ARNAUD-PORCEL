 ...
 * - image: Use atum-file:// protocol (bypasses CSP in renderer)
...
class CanvasLifecycle {
   ...
   * Note: BrowserView can access file:// directly, no need for atum-file://
   */
  ...
  private async loadFileContent(tabId: string, path: string, type: ContentType): Promise<void> {
    ...
    // Images use atum-file:// protocol directly (no content loading needed)
    if (type === 'image') {
      tab.isLoading = false
      this.notifyTabsChange()
      return
  ...
  /**
   * Open the conversation-level diff viewer tab.
   * Reuses existing diff tab if already open.
   */
  async openDiffView(): Promise<string> {
    // Reuse existing diff tab
    for (const [tabId, tab] of this.tabs) {
      if (tab.type === 'diff') {
        await this.switchTab(tabId)
        return tabId
  ...
  /**
   * Open the git integration viewer tab.
   * Reuses existing git tab if already open.
   */
  async openGitView(): Promise<string> {
    // Reuse existing git tab
    for (const [tabId, tab] of this.tabs) {
      if (tab.type === 'git') {
        await this.switchTab(tabId)
        return tabId
