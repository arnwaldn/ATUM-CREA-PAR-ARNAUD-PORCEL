...
import { api } from '../api'
import type { AtumCreConfig, AppView, McpServerStatus } from '../types'
...
interface AppState {
  ...
  updateConfig: (updates: Partial<AtumCreConfig>) => void
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  goBack: () => {
    ...
    // Go back to previous view, or default to chat
  ...
  initialize: async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        const config = response.data as AtumCreConfig
        ...
        if (config.isFirstLaunch || !hasAnyAISource(config.aiSources)) {
          ...
          // Go directly to chat (skip home page)
          console.log('[Store] Config loaded, going to chat')
          set({ view: 'space' })
        }
      } else {
