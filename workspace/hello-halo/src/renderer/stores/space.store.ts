...
export const useSpaceStore = create<SpaceState>((set, get) => ({
  ...
  // Load ATUM CREA temp space
  loadAtumCreSpace: async () => {
    try {
      const response = await api.getAtumCreSpace()

      if (response.success && response.data) {
        set({ atumSpace: response.data as Space })
      }
    } catch (error) {
      console.error('Failed to load ATUM CREA space:', error)
    }
  },
  ...
  loadSpaces: async () => {
    try {
      ...
      // Load both ATUM CREA space and user spaces
      await get().loadAtumCreSpace()
  ...
  updateSpacePreferences: async (spaceId, preferences) => {
    try {
      ...
      if (response.success && response.data) {
        ...
        // Update in spaces list or ATUM CREA space
        if (updatedSpace.isTemp) {
          set({ atumSpace: updatedSpace })
        } else {
          set((state) => ({
            spaces: state.spaces.map((s) =>
              s.id === spaceId ? updatedSpace : s
  ...
  getSpacePreferences: (spaceId) => {
    const { atumSpace, spaces, currentSpace } = get()
    ...
    // Check ATUM CREA space
    if (atumSpace?.id === spaceId) {
      return atumSpace.preferences
    }
