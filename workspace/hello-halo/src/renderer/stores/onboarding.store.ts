 ...
 *
 * Flow:
 * 1. step 1: Highlight ATUM CREA Space card on HomePage
...
export type OnboardingStep = 'atum-space' | 'send-message' | 'view-artifact' | 'completed'
...
export const useOnboardingStore = create<OnboardingState>((set, get) => ({
  ...
  startOnboarding: () => {
    set({ isActive: true, currentStep: 'atum-space' })
  },

  nextStep: () => {
    const { currentStep } = get()

    const stepOrder: OnboardingStep[] = ['atum-space', 'send-message', 'view-artifact', 'completed']
