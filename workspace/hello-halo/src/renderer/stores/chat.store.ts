...
import { api } from '../api'
import type { Conversation, ConversationMeta, Message, MessageVariant, ToolCall, Artifact, Thought, AgentEventBase, ImageAttachment, CompactInfo, CanvasContext, AgentErrorType } from '../types'
...
interface ChatState {
  ...
  // Pending variants: saved during regeneration, attached after completion
  pendingVariants: Map<string, MessageVariant[]>
  ...
  // Working directory override per space: Map<spaceId, path>
  workingDirOverrides: Map<string, string>
  ...
  // Working directory override actions
  setWorkingDirOverride: (spaceId: string, path: string | null) => void
  getWorkingDirOverride: (spaceId: string) => string | undefined
  ...
  // Message actions
  regenerateMessage: (conversationId: string) => Promise<void>
  deleteMessage: (spaceId: string, conversationId: string, messageId: string) => Promise<boolean>
  editAndRegenerate: (conversationId: string, messageId: string, newContent: string) => Promise<void>
  switchVariant: (conversationId: string, messageId: string, variantIndex: number | undefined) => void
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  pendingVariants: new Map<string, MessageVariant[]>(),
  workingDirOverrides: new Map<string, string>(),
  currentSpaceId: null,
  artifacts: [],
  isLoading: false,
  ...
  // Working directory override actions
  setWorkingDirOverride: (spaceId: string, path: string | null) => {
    set((state) => {
      const newOverrides = new Map(state.workingDirOverrides)
      if (path) {
        newOverrides.set(spaceId, path)
      } else {
        newOverrides.delete(spaceId)
      }
      return { workingDirOverrides: newOverrides }
    })
  },

  getWorkingDirOverride: (spaceId: string) => {
    return get().workingDirOverrides.get(spaceId)
  },
  ...
  sendMessage: async (content, images, aiBrowserEnabled, thinkingEnabled) => {
    ...
    try {
      ...
      // Send to agent (with images, AI Browser state, thinking mode, canvas context, and working dir override)
      const workingDirOverride = get().workingDirOverrides.get(currentSpaceId)
      await api.sendMessage({
        ...
        workingDirOverride  // Pass working directory override if set
      })
    } catch (error) {
  ...
  // Regenerate last AI message — re-sends the last user message
  // Saves the old assistant response as a variant for branching navigation
  regenerateMessage: async (conversationId: string) => {
    const { currentSpaceId, conversationCache } = get()
    if (!currentSpaceId) return
    ...
    // Save old assistant response as a variant before truncating
    ...
    if (lastAssistant?.role === 'assistant' && lastAssistant.content) {
      const existingVariants = lastAssistant.variants || []
      ...
      set((state) => {
        const newPending = new Map(state.pendingVariants)
        newPending.set(conversationId, [...existingVariants, variant])
        return { pendingVariants: newPending }
      })
    }

    // Remove all messages after the last user message
    const lastUserIndex = conversation.messages.indexOf(lastUserMessage)
    const trimmedMessages = conversation.messages.slice(0, lastUserIndex)

    // Update backend with trimmed messages
    await api.updateConversation(currentSpaceId, conversationId, {
    ...
    set((state) => {
      const newCache = new Map(state.conversationCache)
      newCache.set(conversationId, { ...conversation, messages: trimmedMessages, messageCount: trimmedMessages.length })
      return { conversationCache: newCache }
    })

    // Re-send the user message
    await get().sendMessage(lastUserMessage.content, lastUserMessage.images)
  },

  // Delete a single message from conversation
  deleteMessage: async (spaceId: string, conversationId: string, messageId: string) => {
    const conversation = get().conversationCache.get(conversationId)
    if (!conversation) return false

    const newMessages = conversation.messages.filter(m => m.id !== messageId)

    // Update backend
    const response = await api.updateConversation(spaceId, conversationId, {
    ...
    if (response.success) {
      set((state) => {
        const newCache = new Map(state.conversationCache)
        newCache.set(conversationId, { ...conversation, messages: newMessages, messageCount: newMessages.length })
        ...
        if (spaceState) {
          newSpaceStates.set(spaceId, {
            ...spaceState,
            conversations: spaceState.conversations.map(c =>
              c.id === conversationId ? { ...c, messageCount: newMessages.length } : c
            )
          })
        }
  ...
  editAndRegenerate: async (conversationId: string, messageId: string, newContent: string) => {
    const { currentSpaceId, conversationCache } = get()
    if (!currentSpaceId) return
    ...
    const trimmedMessages = conversation.messages.slice(0, msgIndex)
  ...
  // Switch variant for a message (conversation branching)
  switchVariant: (conversationId: string, messageId: string, variantIndex: number | undefined) => {
    const { currentSpaceId } = get()
    set((state) => {
      const newCache = new Map(state.conversationCache)
      ...
      const messages = conv.messages.map(m =>
        m.id === messageId ? { ...m, variantIndex } : m
      )
      newCache.set(conversationId, { ...conv, messages })
      return { conversationCache: newCache }
    })
    // Persist to backend
    if (currentSpaceId) {
      const conv = get().conversationCache.get(conversationId)
      if (conv) {
        api.updateConversation(currentSpaceId, conversationId, {
  ...
  handleAgentComplete: async (data) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // Attach pending variants from regeneration (if any)
        const pendingVars = get().pendingVariants.get(conversationId)
        if (pendingVars && pendingVars.length > 0 && updatedConversation.messages.length > 0) {
          const lastMsg = updatedConversation.messages[updatedConversation.messages.length - 1]
          if (lastMsg.role === 'assistant') {
            lastMsg.variants = pendingVars
            // Persist variants to backend
            api.updateConversation(spaceId, conversationId, {
              messages: updatedConversation.messages
            } as any).catch(err => console.error('[ChatStore] Failed to save variants:', err))
          }
        }
...
/**
 * Hook to get the current working directory override for a space
 */
export function useWorkingDirOverride(spaceId: string): string | undefined {
  return useChatStore((state) => state.workingDirOverrides.get(spaceId))
}
