/**
 * CodeMirror Theme for ATUM CREA
 *
 * Creates a theme that matches ATUM CREA's CSS variable-based theme system.
 * The theme automatically adapts to light/dark mode via CSS variables.
 *
 * Design principles:
 * - Use ATUM CREA CSS variables (--background, --foreground, etc.)
...
/**
 * Base editor theme using ATUM CREA CSS variables
 * Uses CSS variables that automatically adapt to light/dark mode
 */
export const atumEditorTheme = EditorView.theme(
  {
 ...
 */
export const atumHighlightStyle = HighlightStyle.define([
  // Comments
...
/**
 * Complete ATUM CREA theme for CodeMirror
 * Combines editor styling and syntax highlighting
 */
export const atumTheme: Extension = [
  atumEditorTheme,
  syntaxHighlighting(atumHighlightStyle),
]

/**
 * Light theme variant (same structure, CSS variables handle the colors)
 * The base atumTheme already uses CSS variables, so it works for both
 */
export const atumLightTheme = atumTheme

/**
 * Dark theme variant
 */
export const atumDarkTheme = atumTheme
