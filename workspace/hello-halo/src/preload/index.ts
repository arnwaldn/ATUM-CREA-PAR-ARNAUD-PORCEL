...
export interface AtumCreAPI {
  ...
  // Space
  getAtumCreSpace: () => Promise<IpcResponse>
  ...
  // MCP OAuth
  mcpOAuthStart: (serverName: string, mcpServerUrl: string) => Promise<IpcResponse>
  mcpOAuthDisconnect: (serverName: string) => Promise<IpcResponse>
  mcpOAuthRefresh: (serverName: string) => Promise<IpcResponse>
  onMcpOAuthResult: (callback: (data: {
    serverName: string
    success: boolean
    error?: string
  }) => void) => () => void
  ...
  // Git (read-only operations for Canvas)
  gitIsRepo: (spaceId: string) => Promise<IpcResponse<boolean>>
  gitStatus: (spaceId: string) => Promise<IpcResponse>
  gitBranches: (spaceId: string) => Promise<IpcResponse>
  gitLog: (spaceId: string, count?: number) => Promise<IpcResponse>
  gitDiff: (spaceId: string, file?: string) => Promise<IpcResponse>
  gitDiffStaged: (spaceId: string, file?: string) => Promise<IpcResponse>
...
const api: AtumCreAPI = {
  ...
  // Space
  getAtumCreSpace: () => ipcRenderer.invoke('space:get-default'),
  ...
  // MCP OAuth
  mcpOAuthStart: (serverName, mcpServerUrl) => ipcRenderer.invoke('mcp-oauth:start', serverName, mcpServerUrl),
  mcpOAuthDisconnect: (serverName) => ipcRenderer.invoke('mcp-oauth:disconnect', serverName),
  mcpOAuthRefresh: (serverName) => ipcRenderer.invoke('mcp-oauth:refresh', serverName),
  onMcpOAuthResult: (callback) => createEventListener('mcp-oauth:result', callback as (data: unknown) => void),
  ...
  // Voice (Speech-to-Text)
  transcribeAudio: (request) => ipcRenderer.invoke('voice:transcribe', request),

  // Git (read-only operations for Canvas)
  gitIsRepo: (spaceId) => ipcRenderer.invoke('git:is-repo', spaceId),
  gitStatus: (spaceId) => ipcRenderer.invoke('git:status', spaceId),
  gitBranches: (spaceId) => ipcRenderer.invoke('git:branches', spaceId),
  gitLog: (spaceId, count) => ipcRenderer.invoke('git:log', spaceId, count),
  gitDiff: (spaceId, file) => ipcRenderer.invoke('git:diff', spaceId, file),
  gitDiffStaged: (spaceId, file) => ipcRenderer.invoke('git:diff-staged', spaceId, file),
...
contextBridge.exposeInMainWorld('atumCrea', api)
...
// This is used by the overlay window which doesn't need the full ATUM CREA API
const electronAPI = {
  ipcRenderer: {
...
// TypeScript declaration for window.atumCrea and window.platform
declare global {
  interface Window {
    atumCrea: AtumCreAPI
    platform: {
