/**
 * ATUM CREA - Electron Main Process
...
// Logs are written to: ~/Library/Logs/ATUM CREA/ (macOS), %USERPROFILE%\AppData\Roaming\ATUM CREA\logs (Windows)
    ...
    // Help menu (Windows: includes Check for Updates)
    {
      ...
      submenu: [
        ...
        //   : []),
        {
          label: 'Learn More',
          click: async () => {
            await shell.openExternal('https://github.com/atumcrea/atum-crea')
          }
        }
      ]
    }
...
app.whenReady().then(async () => {
  ...
  electronApp.setAppUserModelId('com.atumcrea.app')

  // Register custom protocols (atum-file://, etc.)
  registerProtocols()
