...
import { getAtumCreDir, getTempSpacePath, getSpacesDir } from './config.service'
...
function getSpaceIndexPath(): string {
  return join(getAtumCreDir(), 'spaces-index.json')
}
...
// ============================================================================
// ATUM CREA Temp Space
// ============================================================================

const ATUM_SPACE: Space = {
  id: 'atum-temp',
  name: 'ATUM CREA',
  icon: 'sparkles',
  path: '',
...
export function getAtumCreSpace(): Space {
...
export function deleteSpace(spaceId: string): boolean {
  ...
  try {
    if (isCustomPath) {
      ...
      const appDir = join(spacePath, '.halo')
...
export function updateSpacePreferences(
  ...
  try {
    ...
    const appDir = join(space.path, '.halo')
    if (!existsSync(appDir)) {
