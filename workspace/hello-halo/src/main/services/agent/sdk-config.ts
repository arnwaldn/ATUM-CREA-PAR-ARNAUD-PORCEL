...
import { isClaudeCodeNativeKey, getClaudeConfigDir } from '../claude-code-auth'
...
export async function resolveCredentialsForSdk(
  ...
  let sdkModel = credentials.model || 'claude-opus-4-6'
...
export function buildSdkEnv(params: SdkEnvParams): Record<string, string | number> {
  const isNativeAuth = isClaudeCodeNativeKey(params.anthropicApiKey as string)
  ...
  // Enable Agent Teams (research preview) if configured
  if (params.agentTeams !== false) {
    env.CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS = '1'
  }

  if (isNativeAuth) {
    // Claude Code native auth: use real ~/.claude/ config dir
    // The CLI subprocess will find OAuth tokens there and use the subscription
    // Do NOT set ANTHROPIC_API_KEY - that would trigger paid API usage
    env.CLAUDE_CONFIG_DIR = getClaudeConfigDir()
    console.log('[SDK Config] Using Claude Code native auth (subscription)')
  } else {
    // Standard provider auth: pass API key and use isolated config dir
    env.ANTHROPIC_API_KEY = params.anthropicApiKey
    env.ANTHROPIC_BASE_URL = params.anthropicBaseUrl
    env.CLAUDE_CONFIG_DIR = path.join(app.getPath('userData'), 'claude-config')
  }

  // ATUM CREA: Inject ECC env vars (agents, rules, skills, commands)
  const eccVars = getECCEnvVars('coding')
  Object.assign(env, eccVars)
...
export function buildBaseSdkOptions(params: BaseSdkOptionsParams): Record<string, any> {
  ...
  const sdkOptions: Record<string, any> = {
    ...
    // Use ATUM CREA's custom system prompt instead of SDK's 'claude_code' preset
    systemPrompt: buildSystemPrompt({ workDir, modelInfo: credentials.displayModel, thinkingEnabled: params.thinkingEnabled }),
    maxTurns: 200,
