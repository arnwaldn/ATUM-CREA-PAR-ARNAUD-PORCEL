 ...
 * ~/Library/Application Support/atum-crea/claude-config/ (via CLAUDE_CONFIG_DIR env)
 * to isolate ATUM CREA from user's own Claude Code configuration.
 *
 ...
 *   e.g., /Volumes/one_tb/code2/project -> -Volumes-one-tb-code2-project
 *
...
export function needsSessionRebuild(existing: V2SessionInfo, newConfig: SessionConfig): boolean {
  if (existing.config.aiBrowserEnabled !== newConfig.aiBrowserEnabled) return true
  // Rebuild when working directory override changes (e.g., user selected a different folder)
  if (newConfig.workDir && existing.config.workDir !== newConfig.workDir) return true
  // Rebuild when override is cleared (workDir was set, now it's not)
  if (existing.config.workDir && !newConfig.workDir) return true
  return false
}
