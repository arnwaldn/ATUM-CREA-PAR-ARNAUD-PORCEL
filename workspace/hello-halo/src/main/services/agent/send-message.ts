...
import { refreshExpiredMcpTokens } from '../mcp-oauth'
import {
  formatCanvasContext,
  buildMessageContent,
  parseSDKMessage,
...
import { RetriableError, classifyError, circuitBreaker } from './error-classifier'

// Unified fallback error suffix - guides user to check logs
const FALLBACK_ERROR_HINT = 'Check logs in Settings > System > Logs.'

// Retry configuration for transient API errors (concurrency, rate limits)
const MAX_RETRIES = 2
const BASE_RETRY_DELAY_MS = 2000  // 2s, 4s with exponential backoff
...
/**
 * Send message to agent with automatic retry for transient API errors.
 *
 * Wraps sendMessageInternal with retry logic:
 * - Detects retriable errors (concurrency, rate limits, overloaded)
 * - Retries up to MAX_RETRIES times with exponential backoff
 * - Preserves V2 session across retries
 * - Notifies renderer during retry wait
 */
export async function sendMessage(
  mainWindow: BrowserWindow | null,
  request: AgentRequest
): Promise<void> {
  let disableMcpOnRetry = false

  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      await sendMessageInternal(mainWindow, request, attempt, { disableMcpOnRetry })
      circuitBreaker.reset()  // Success — reset circuit breaker
      return
    } catch (error: unknown) {
      // Don't retry aborts
      if ((error as Error).name === 'AbortError') return

      // Only retry RetriableError (with circuit breaker protection)
      if (error instanceof RetriableError && attempt < MAX_RETRIES) {
        // Circuit breaker: stop retrying if the same error keeps occurring
        if (circuitBreaker.record(error.errorType)) {
          console.log(`[Agent][${request.conversationId}] Circuit breaker tripped for "${error.errorType}" — stopping retries`)
          sendToRenderer('agent:error', request.spaceId, request.conversationId, {
            type: 'error',
            error: `Repeated error (${error.errorType}). Please try again later.`
          })
          return
        }
        ...
        // Schema validation errors: close session and retry WITHOUT MCP servers
        if (error.errorType === 'schema_validation') {
          console.log(`[Agent][${request.conversationId}] Schema validation error - will retry without MCP servers`)
          closeV2Session(request.conversationId)
          disableMcpOnRetry = true

          sendToRenderer('agent:error', request.spaceId, request.conversationId, {
            type: 'error',
            errorType: 'retry',
            error: `MCP tool schema incompatible, retrying without MCP servers (${nextAttempt}/${MAX_RETRIES})...`
          })
        } else {
          console.log(`[Agent][${request.conversationId}] Retriable error "${error.errorType}" (attempt ${nextAttempt}/${MAX_RETRIES}), retrying in ${delay}ms...`)

          sendToRenderer('agent:error', request.spaceId, request.conversationId, {
            type: 'error',
            errorType: 'retry',
            error: `API busy (${error.errorType}), retrying (${nextAttempt}/${MAX_RETRIES})...`
          })
        }
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          if (timer) void 0  // Suppress unused warning
        })

        continue
      }

      // Non-retriable or max retries exhausted: propagate
      throw error
    }
  }
}
...
/**
 * Internal message sending logic (called by sendMessage wrapper).
 */
async function sendMessageInternal(
  mainWindow: BrowserWindow | null,
  request: AgentRequest,
  attemptNumber: number,
  options: { disableMcpOnRetry?: boolean } = {}
): Promise<void> {
  ...
  console.log(`[Agent] sendMessage: conv=${conversationId}${attemptNumber > 0 ? ` (retry #${attemptNumber})` : ''}${images && images.length > 0 ? `, images=${images.length}` : ''}${aiBrowserEnabled ? ', AI Browser enabled' : ''}${thinkingEnabled ? ', thinking=ON' : ''}${canvasContext?.isOpen ? `, canvas tabs=${canvasContext.tabCount}` : ''}${workingDirOverride ? `, workDir=${workingDirOverride}` : ''}`)

  const config = getConfig()
  const workDir = getWorkingDir(spaceId, workingDirOverride)
  ...
  try {
    ...
    await refreshExpiredMcpTokens()

    // Get enabled MCP servers (with OAuth tokens injected as headers)
    // On schema_validation retry, skip all user MCP servers to avoid invalid tool schemas
    const enabledMcpServers = options.disableMcpOnRetry
      ? null
      : getEnabledMcpServers(config.mcpServers || {})

    if (options.disableMcpOnRetry) {
      console.log(`[Agent][${conversationId}] MCP servers disabled for this retry (schema_validation recovery)`)
    }
    ...
    // Config Manager MCP server (always available — lets Claude manage MCP config from chat)
    mcpServers['config-manager'] = createConfigManagerMcpServer()

    // Hindsight: Native self-learning memory engine (always available in every session)
    const { createHindsightMcpServer } = await import('../hindsight')
    mcpServers['hindsight'] = createHindsightMcpServer()
    console.log(`[Agent][${conversationId}] Hindsight native memory engine added`)

    // MCP Orchestrator: Dynamic MCP activation/deactivation (always available)
    mcpServers['mcp-orchestrator'] = createMcpOrchestratorServer()
    console.log(`[Agent][${conversationId}] MCP Orchestrator added`)
    ...
    const sdkOptions = buildBaseSdkOptions({
      ...
      agentTeams: config.system?.agentTeams,
    ...
    if (aiBrowserEnabled) {
      sdkOptions.systemPrompt = buildSystemPromptWithAIBrowser(
        { workDir, modelInfo: resolvedCredentials.displayModel, thinkingEnabled: !!thinkingEnabled },
        AI_BROWSER_SYSTEM_PROMPT
      )
    }
    if (thinkingEnabled) {
      sdkOptions.maxThinkingTokens = 32000
    }
    ...
    const sessionConfig: SessionConfig = {
      ...
      workDir: workingDirOverride || undefined
    }
    ...
    try {
      ...
      if (v2Session.setMaxThinkingTokens) {
        await v2Session.setMaxThinkingTokens(thinkingEnabled ? 32000 : null)
        console.log(`[Agent][${conversationId}] Thinking mode: ${thinkingEnabled ? 'ON (32000 tokens)' : 'OFF'}`)
      }
    } catch (e) {
      console.error(`[Agent][${conversationId}] Failed to set dynamic params:`, e)
    }

    // Give the MCP orchestrator a reference to the live session for dynamic setMcpServers()
    ...
    // Retriable errors: re-throw to trigger retry in wrapper (DON'T close session)
    if (err instanceof RetriableError) {
      console.log(`[Agent][${conversationId}] Retriable error: ${err.errorType} - preserving V2 session`)
      throw err
    }

    console.error(`[Agent][${conversationId}] Fatal error:`, error)
    ...
    // Close V2 session ONLY for fatal errors (retriable errors preserve session above)
    closeV2Session(conversationId)
  } finally {
    // Release orchestrator's session reference (prevents stale references)
  ...
  for await (const sdkMessage of v2Session.stream()) {
    ...
    if (thought) {
      ...
      if (thought.type === 'tool_result') {
        ...
        if (thought.type === 'text') {
          ...
          // Classify error: retriable (concurrency, rate limit) vs fatal (auth, billing)
          const classification = classifyError(thought.content, thought.errorCode)
          console.log(`[Agent][${conversationId}] Error thought: "${thought.content}" → ${classification.errorType} (retriable: ${classification.isRetriable})`)

          if (classification.isRetriable) {
            // Checkpoint: save partial content before retry so work isn't lost
            const partialContent = lastTextContent || currentStreamingText || ''
            if (partialContent || sessionState.thoughts.length > 0) {
              updateLastMessage(spaceId, conversationId, {
                content: partialContent,
                thoughts: [...sessionState.thoughts]
              })
              console.log(`[Agent][${conversationId}] Checkpoint saved before retry: ${partialContent.length} chars, ${sessionState.thoughts.length} thoughts`)
            }
            // Throw to break stream loop and trigger retry in sendMessage wrapper
            throw new RetriableError(thought.content, classification.errorType)
          }

          // Fatal error - send to frontend immediately
          sendToRenderer('agent:error', spaceId, conversationId, {
            type: 'error',
    ...
    if (sdkMessage.type === 'system') {
      ...
      if (mcpServers && mcpServers.length > 0) {
        ...
        // Notify renderer about servers that need OAuth authentication
        const needsAuth = mcpServers.filter(s => s.status === 'needs-auth' || s.status === 'failed')
        if (needsAuth.length > 0) {
          const needsAuthNames = needsAuth.map(s => s.name)
          console.warn(`[Agent][${conversationId}] MCP servers need attention:`, needsAuthNames)
        }
      }
      ...
      if (isError || hasErrors) {
        ...
        // Check if this is a retriable error (concurrency, rate limit, etc.)
        const resultClassification = classifyError(realError, (sdkMessage as any).subtype)
        if (resultClassification.isRetriable) {
          // Checkpoint: save partial content before retry
          const partialContent = lastTextContent || currentStreamingText || ''
          if (partialContent || sessionState.thoughts.length > 0) {
            updateLastMessage(spaceId, conversationId, {
              content: partialContent,
            ...
            console.log(`[Agent][${conversationId}] Checkpoint saved before result retry: ${partialContent.length} chars, ${sessionState.thoughts.length} thoughts`)
          }
          console.log(`[Agent][${conversationId}] Result error is retriable: ${resultClassification.errorType}`)
          throw new RetriableError(realError, resultClassification.errorType)
        }
