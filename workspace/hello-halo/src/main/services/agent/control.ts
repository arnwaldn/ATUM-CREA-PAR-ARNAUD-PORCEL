...
export async function stopGeneration(conversationId?: string): Promise<void> {
  if (conversationId) {
    ...
    if (session) {
      ...
      // Interrupt V2 Session and drain stale messages (with timeout)
      ...
      if (v2Session) {
        try {
          ...
          // Drain stale messages with 5-second timeout to prevent UI blocking
          ...
          for await (const msg of v2Session.session.stream()) {
            ...
            if (Date.now() - drainStart > drainTimeout) {
              console.warn(`[Agent] Drain timeout reached (${drainTimeout}ms) — stopping drain`)
              break
            }
          }
