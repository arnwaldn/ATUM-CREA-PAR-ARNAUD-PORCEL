...
await window.atumCrea.perfStart()   // Start monitoring
await window.atumCrea.perfStop()    // Stop monitoring
```
...
```javascript
await window.atumCrea.perfStart({
...
```javascript
await window.atumCrea.perfGetState()     // Get current state
await window.atumCrea.perfClearHistory() // Clear history
await window.atumCrea.perfExport()       // Export as JSON
```
