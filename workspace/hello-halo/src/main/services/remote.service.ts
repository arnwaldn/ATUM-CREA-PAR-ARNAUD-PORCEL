...
import { addFirewallRule, removeFirewallRule, hasFirewallRule } from './firewall'
...
export async function enableRemoteAccess(
  ...
  try {
    const firewallResult = await addFirewallRule(actualPort)
    if (firewallResult.success) {
      console.log(`[Remote] Firewall rule added for port ${actualPort}${firewallResult.elevated ? ' (elevated)' : ''}`)
    } else {
      console.warn(`[Remote] Could not add firewall rule - LAN access may be blocked. Users can use the Cloudflare tunnel as an alternative.`)
    }
  } catch (error) {
    console.warn('[Remote] Firewall rule setup failed:', error)
  }
...
export async function disableRemoteAccess(): Promise<void> {
  ...
  try {
    ...
    console.log('[Remote] Firewall rule removed')
  } catch (error) {
    console.warn('[Remote] Could not remove firewall rule:', error)
  }
