 ...
 * 2. Args-based cleanup (scan for --atum-managed flag)
 *
...
// Command-line argument patterns for ATUM CREA-managed processes
const MANAGED_FLAG = 'atum-managed'
const INSTANCE_PREFIX = 'atum-instance='
...
export async function cleanupOrphans(): Promise<CleanupResult> {
  ...
  try {
    // Find all processes with --atum-managed flag
    const managedProcesses = await platformOps.findByArgs(MANAGED_FLAG)
    console.log(`[Health][Cleaner] Found ${managedProcesses.length} ATUM CREA-managed processes by args scan`)

    for (const proc of managedProcesses) {
      // Only kill if NOT current instance
      if (proc.commandLine.includes(`${INSTANCE_PREFIX}${currentInstanceId}`)) {
...
/**
 * Check if a specific process is an ATUM CREA-managed process
 */
export async function isAtumCreManagedProcess(pid: number): Promise<boolean> {
  const platformOps = getPlatformOps()

  try {
    const processes = await platformOps.findByArgs(MANAGED_FLAG)
...
/**
 * Get all running ATUM CREA-managed processes
 */
export async function getRunningAtumCreProcesses(): Promise<Array<{
  pid: number
  ...
  try {
    ...
    return processes.map(proc => {
      ...
      const instanceMatch = proc.commandLine.match(new RegExp(`${INSTANCE_PREFIX}([a-f0-9-]+)`))
...
export async function verifyCleanup(): Promise<boolean> {
  const currentInstanceId = getCurrentInstanceId()
  const runningProcesses = await getRunningAtumCreProcesses()
