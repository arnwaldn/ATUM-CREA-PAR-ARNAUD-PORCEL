...
function migrateEncryptedCredentials(): void {
  ...
  if (result.migrated) {
    try {
      ...
      _configCache = null  // Invalidate cache after migration write
...
let _configCache: AtumCreConfig | null = null
...
export function getAtumCreDir(): string {
  ...
  //    Keep ~/.halo-dev path for backward compatibility with existing user data
  if (!app.isPackaged) {
    return join(homedir(), '.halo-dev')
  }

  // 3. Production: use default directory
  //    Keep ~/.halo path for backward compatibility with existing user data
  return join(homedir(), '.halo')
}

export function getConfigPath(): string {
  return join(getAtumCreDir(), 'config.json')
}

export function getTempSpacePath(): string {
  return join(getAtumCreDir(), 'temp')
}

export function getSpacesDir(): string {
  return join(getAtumCreDir(), 'spaces')
}
...
const DEFAULT_CONFIG: AtumCreConfig = {
  api: {
    provider: 'anthropic',
    apiKey: '',
...
export async function initializeApp(): Promise<void> {
  ...
  const dirs = [appDir, tempDir, spacesDir, tempArtifactsDir, tempConversationsDir]
  ...
  if (!existsSync(configPath)) {
    ...
    _configCache = null  // Invalidate cache after initial config write
  }
...
// Get configuration
export function getConfig(): AtumCreConfig {
...
// Save configuration
export function saveConfig(config: Partial<AtumCreConfig>): AtumCreConfig {
