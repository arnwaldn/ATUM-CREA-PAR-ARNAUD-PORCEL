...
import { getTempSpacePath, getAtumCreDir } from './config.service'
...
export class SearchService {
  ...
  private async searchFile(filePath: string, query: string): Promise<SearchResult[]> {
    ...
    try {
      ...
      // Get space name
      let spaceName = data.spaceId === 'atum-temp' ? 'ATUM CREA' : data.spaceId

      try {
        if (data.spaceId !== 'atum-temp') {
  ...
  private getFilesToSearch(
    ...
    if (scope === 'global') {
      ...
      const spacesDir = join(appDir, 'spaces')
