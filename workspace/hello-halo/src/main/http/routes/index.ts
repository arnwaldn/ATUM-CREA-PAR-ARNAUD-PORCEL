...
export function registerApiRoutes(app: Express, mainWindow: BrowserWindow | null): void {
  ...
  // ===== Space Routes =====
  app.get('/api/spaces/default', async (req: Request, res: Response) => {
    const result = spaceController.getAtumTempSpace()
    res.json(result)
  })
  ...
  app.post('/api/agent/message', async (req: Request, res: Response) => {
    const { spaceId, conversationId, message, resumeSessionId, images, thinkingEnabled, aiBrowserEnabled, workingDirOverride } = req.body
    const result = await agentController.sendMessage(mainWindow, {
      ...
      workingDirOverride  // Pass working directory override
    })
    res.json(result)
  })
  ...
  app.get('/api/spaces/:spaceId/artifacts/download-all', async (req: Request, res: Response) => {
    try {
      ...
      const fileName = spaceId === 'atum-temp' ? 'atum-artifacts' : basename(workDir)
      res.setHeader('Content-Type', 'application/gzip')
      res.setHeader('Content-Disposition', `attachment; filename="${fileName}.tar.gz"`)
