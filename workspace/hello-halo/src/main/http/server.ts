/**
 * HTTP Server - Remote access server for ATUM CREA
...
import { handleCallback as handleOAuthCallback } from '../services/mcp-oauth'
...
export async function startHttpServer(
  ...
  // CORS for remote access — reflect request origin instead of wildcard
  ...
  // OAuth MCP callback (public - browser redirects here after authorization)
  expressApp.get('/oauth/mcp/callback', async (req: Request, res: Response) => {
    const { code, state, error, error_description } = req.query

    if (error) {
      const msg = (error_description || error) as string
      console.error('[HTTP] OAuth callback error:', msg)
      res.status(400).send(getOAuthResultPage(false, msg))
      return
    }

    if (!code || !state) {
      res.status(400).send(getOAuthResultPage(false, 'Missing code or state parameter'))
      return
    }

    const result = await handleOAuthCallback(code as string, state as string)
    res.send(getOAuthResultPage(result.success, result.error, result.serverName))
  })
  ...
  if (is.dev) {
    ...
    expressApp.use('/{*path}', (req, res) => {
      ...
      if (req.path === '/' && !urlToken && !headerToken) {
        ...
        const hasToken = cookies.includes('atum_authenticated=true')
        if (!hasToken) {
  ...
  // Initialize WebSocket (for ATUM CREA communication on /ws path)
  ...
  if (is.dev) {
    httpServer.on('upgrade', (req, socket, head) => {
      ...
      // Don't intercept ATUM CREA's WebSocket connections
      if (url.pathname === '/ws') {
 ...
 */
function getOAuthResultPage(success: boolean, error?: string, serverName?: string): string {
  const title = success ? 'Connected' : 'Authentication Failed'
  const message = success
    ? `Successfully connected to <strong>${serverName || 'MCP server'}</strong>. You can close this tab.`
    : `Authentication failed: ${error || 'Unknown error'}. Please try again from ATUM CREA settings.`
  const color = success ? '#22c55e' : '#ef4444'
  const icon = success ? '&#10003;' : '&#10007;'

  return `<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>ATUM CREA - ${title}</title>
<style>
  ...
  .card { text-align: center; padding: 3rem; border-radius: 1rem;
    background: #1e293b; box-shadow: 0 4px 24px rgba(0,0,0,0.3); max-width: 480px; }
  .icon { font-size: 3rem; color: ${color}; margin-bottom: 1rem; }
  h1 { font-size: 1.5rem; margin: 0 0 1rem; color: ${color}; }
...
<head>
  ...
  <title>ATUM CREA Remote Access</title>
  <style>
  ...
  <script>
    async function login() {
      ...
      try {
        ...
        if (res.ok) {
          localStorage.setItem('atum_remote_token', token);
          // Set cookie for server-side auth check
          document.cookie = 'atum_authenticated=true; path=/';
          error.textContent = '';
