# ATUM CREA — Project Context (AI Developer Required Reading)

> Purpose: The shortest onboarding document for AI developers working on ATUM CREA. Read this page, then refer to `ARCHITECTURE.md` (technical architecture) and `CHANGELOG.md` (recent changes) before starting work.
...
### ATUM CREA's Positioning

> Turn Claude Code — a "DOS-era AI" — into a "Windows-era AI companion"

ATUM CREA = Claude Code SDK capabilities + ChatGPT-level UI/UX

**Core value**: Wrap complex technical concepts (Agent Loop/CLI) into an intuitive human interaction model.

### Product Naming

- **ATUM CREA** (the product)
- **ATUM CREA Space** (temporary workspace) — "Wandering moments where ideas crystallize"
...
- **Shipped modules**:
  - **Space**: ATUM CREA temporary space + Dedicated spaces (custom paths)
...
- **ATUM CREA Space (temporary)**: Default workspace, `~/.halo/temp/`
