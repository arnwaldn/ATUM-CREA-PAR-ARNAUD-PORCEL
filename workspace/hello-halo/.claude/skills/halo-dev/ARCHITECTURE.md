...
│   │
│   ├── preload/
│   │   └── index.ts               # ~300 lines, exposes AtumCreAPI
│   │
...
| Type | Description |
|------|-------------|
| `AtumCreConfig` | App config: `api`, `aiSources`, `permissions`, `appearance`, `system`, `remoteAccess`, `mcpServers` |
...
|------|---------|---------|
| `src/preload/index.ts` | Expose listener to `window.atumCrea` | `onAgentMcpStatus: (cb) => createEventListener('agent:mcp-status', cb)` |
...
Third-party Providers go in `atum-local/src/providers/`, dynamically loaded via `product.json`.
...
export const api = {
  getConfig: async () => {
    if (isElectron()) return window.atumCrea.getConfig()  // IPC
...
ATUM CREA uses the local filesystem for data storage (no external database/backend):
...
├── temp/                       # ATUM CREA temporary space (id: halo-temp)
...
Anti-flash: `index.html` inline script reads `localStorage('atum-theme')`
