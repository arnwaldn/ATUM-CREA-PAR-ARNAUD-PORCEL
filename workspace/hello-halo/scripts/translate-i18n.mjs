 ...
 *   ATUM_TEST_API_KEY    - API Key
 *   ATUM_TEST_API_URL    - API URL
 *   ATUM_TEST_MODEL      - Model name
 *   ATUM_TEST_API_FORMAT - API format: 'anthropic' or 'openai' (default: auto-detect)
 */
...
const APP_CONTEXT = `
## Product Context
ATUM CREA is a desktop AI assistant application (similar to Cursor/Claude Desktop).
...
function detectApiFormat(apiUrl) {
  const explicit = process.env.ATUM_TEST_API_FORMAT?.toLowerCase()
...
async function translateBatch(texts, targetLocale) {
  const apiKey = process.env.ATUM_TEST_API_KEY
  const apiUrl = process.env.ATUM_TEST_API_URL
  const model = process.env.ATUM_TEST_MODEL

  if (!apiKey || !apiUrl) {
    throw new Error('Missing ATUM_TEST_API_KEY or ATUM_TEST_API_URL in .env.local')
  }
...
3. Keep brand name "ATUM CREA" untranslated
...
async function main() {
  ...
  console.log(`   API: ${process.env.ATUM_TEST_API_URL}`)
  console.log(`   Model: ${process.env.ATUM_TEST_MODEL}`)
  console.log(`   Format: ${detectApiFormat(process.env.ATUM_TEST_API_URL)}\n`)
